...
@Service("realRFC")
@Primary
public class MsReporteCierreAnualRepositoryRFCImpl implements MsReporteCierreAnualRepository {

    public static final String ANIO_CICLO_CASO = "anioCicloCaso";
    public static final String ANIO_REVISION = "anioRevision";
    public static final String OOAD_REGISTRO_PATRONAL = "ooadRegistroPatronal";
    ...
    public static final String CVE_CONSECUENCIA = "cveConsecuencia";
    ...
    public static final String REGISTRO_PATRONAL = "registroPatronal";
    ...
    public List<Integer> ciclosGlobales(int anioRevision){
        AggregationResults<BasicDBObject> aggregacion4 = operations.aggregate(newAggregation(RespaldosRiesgosDescartadosRFC.class,
                        match(where(ANIO_REVISION).is(anioRevision)),
                        group()
                                .addToSet(ANIO_CICLO_CASO).as(CICLOS)
                ), RespaldosRiesgosDescartadosRFC.class,
                BasicDBObject.class
        );


        List<Integer> ciclos = (List<Integer>) validarListNull( aggregacion4.getMappedResults(),CICLOS).get(CICLOS);
        ciclos.sort(Integer::compareTo);
        return ciclos;
    }

    @Override
    public List<ParamModel> getCausisticaPeriodoRevisionTipoRiesgo(int cicloActual, int ooad, List<Integer> subdelegaciones) {
        AggregationResults<BasicDBObject> aggregacion1 = operations.aggregate(newAggregation(RespaldoCierreAnualCasuisticaRFC.class,
                        match(where(OOAD_REGISTRO_PATRONAL).is(ooad).and(ANIO_REVISION).is(cicloActual).and(TIPO_RIESGO).in(
                                Arrays.asList(1, 3)
                        ...
                        group()
                                ...
                                .sum(ConditionalOperators.Cond.newBuilder()
                                        .when(Criteria.where(CVE_CONSECUENCIA).is(4)).then(1).otherwise(0)
                                ).as(TOTAL_DEFUNCIONES)
                ), RespaldoCierreAnualCasuisticaRFC.class,
                BasicDBObject.class
        );
        ...
        if(!mappedResults.isEmpty()){
            ...
            basicDBObject.append(DIAS_SUBSIDIADOS,0);
            basicDBObject.append(PORCENTAJE_INCAPACIDAD,0);
            basicDBObject.append(TOTAL_DEFUNCIONES,0);
        }
        ...
        AggregationResults<BasicDBObject> aggregacion2 = operations.aggregate(newAggregation(RespaldoCierreAnualCasuisticaRFC.class,
                        match(where(OOAD_REGISTRO_PATRONAL).is(ooad).and(ANIO_REVISION).is(cicloActual).and(TIPO_RIESGO).in(
                                Arrays.asList(2)
                        ).and(SUB_DEL_REGISTRO_PATRONAL).in(subdelegaciones)),
                        group()
    ...
    public List<ParamModel> getCausisticaPeriodoRevisionEstadoRegistro(int anioRevision, int ooad, List<Integer> subdelegaciones) {
        ...
        AggregationResults<BasicDBObject> aggregacion1 = operations.aggregate(newAggregation(RespaldoCierreAnualCasuisticaRFC.class,
                        match(where(OOAD_REGISTRO_PATRONAL).is(ooad).and(ANIO_REVISION).is(anioRevision).
        ...
        AggregationResults<BasicDBObject> aggregacion3 = operations.aggregate(newAggregation(RespaldoCierreAnualCasuisticaRFC.class,
                        match(where(OOAD_REGISTRO_PATRONAL).is(ooad).and(ANIO_REVISION).is(anioRevision).
                                and(CLAVE_ESTADO_RIESGO).is(
        ...
        AggregationResults<BasicDBObject> aggregacion4 = operations.aggregate(newAggregation(RespaldoCierreAnualCasuisticaRFC.class,
                        match(where(OOAD_REGISTRO_PATRONAL).is(ooad).and(ANIO_REVISION).is(anioRevision).
                                and(CLAVE_ESTADO_RIESGO).is(
    ...
    public List<Integer> getCiclosCorrectos(int anioRevision, int ooad, List<Integer> subdelegaciones) {
        AggregationResults<BasicDBObject> aggregacion4 = operations.aggregate(newAggregation(RespaldosRiesgosDescartadosRFC.class,
                        match(where(OOAD_REGISTRO_PATRONAL).is(ooad).and(ANIO_REVISION).is(anioRevision).and(SUB_DEL_REGISTRO_PATRONAL).in(subdelegaciones)
    ...
    public List<String> getCicloAnterioresCasosValues(List<Integer> ciclos, int ooad, List<Integer> subdelegaciones) {
        ...
        ciclos.stream().forEach(c -> {
            AggregationResults<BasicDBObject> aggregacion4 = operations.aggregate(newAggregation(RespaldosRiesgosDescartadosRFC.class,
                            match(where(OOAD_REGISTRO_PATRONAL).is(ooad).and(ANIO_CICLO_CASO).is(c)
                                    .and(SUB_DEL_REGISTRO_PATRONAL).in(subdelegaciones)
                            ),
                            group()
                                    .count().as(TOTAL)
    ...
    public List<ParamModel> getCausisticaPeriodoRevisionOtrosEstadosRegistro(int cicloActual, int ooad, List<Integer> subdelegaciones) {
        ...
        AggregationResults<BasicDBObject> aggregacion1 = operations.aggregate(newAggregation(RespaldosRiesgosDescartadosRFC.class,
                        match(where(OOAD_REGISTRO_PATRONAL).is(ooad).and(ANIO_REVISION).is(cicloActual).
                                and(CLAVE_ESTADO_RIESGO).is(
        ...
        AggregationResults<BasicDBObject> aggregacion2 = operations.aggregate(newAggregation(RespaldosRiesgosDescartadosRFC.class,
                        match(where(OOAD_REGISTRO_PATRONAL).is(ooad).and(ANIO_REVISION).is(cicloActual).
                                and(CLAVE_ESTADO_RIESGO).is(
        ...
        AggregationResults<BasicDBObject> aggregacion3 = operations.aggregate(newAggregation(RespaldosRiesgosDescartadosRFC.class,
                        match(where(OOAD_REGISTRO_PATRONAL).is(ooad).and(ANIO_REVISION).is(cicloActual).
                                and(CLAVE_ESTADO_RIESGO).is(
        ...
        AggregationResults<BasicDBObject> aggregacion5 = operations.aggregate(newAggregation(RespaldosRiesgosDescartadosRFC.class,
                        match(where(OOAD_REGISTRO_PATRONAL).is(ooad).and(ANIO_REVISION).is(cicloActual).
                                and(CLAVE_ESTADO_RIESGO).is(
        ...
        AggregationResults<BasicDBObject> aggregacion6 = operations.aggregate(newAggregation(RespaldosRiesgosDescartadosRFC.class,
                        match(where(OOAD_REGISTRO_PATRONAL).is(ooad).and(ANIO_REVISION).is(cicloActual).
                                and(CLAVE_ESTADO_RIESGO).is(
        ...
        AggregationResults<BasicDBObject> aggregacion7 = operations.aggregate(newAggregation(RespaldosRiesgosDescartadosRFC.class,
                        match(where(OOAD_REGISTRO_PATRONAL).is(ooad).and(ANIO_REVISION).is(cicloActual).
                                and(CLAVE_ESTADO_RIESGO).is(
        ...
        AggregationResults<BasicDBObject> aggregacion8 = operations.aggregate(newAggregation(RespaldosRiesgosDescartadosRFC.class,
                        match(where(OOAD_REGISTRO_PATRONAL).is(ooad).and(ANIO_REVISION).is(cicloActual).
                                and(CLAVE_ESTADO_RIESGO).is(
        ...
        AggregationResults<BasicDBObject> aggregacion9 = operations.aggregate(newAggregation(RespaldosRiesgosDescartadosRFC.class,
                        match(where(OOAD_REGISTRO_PATRONAL).is(ooad).and(ANIO_REVISION).is(cicloActual).
                                and(CLAVE_ESTADO_RIESGO).is(
        ...
        AggregationResults<BasicDBObject> aggregacion10 = operations.aggregate(newAggregation(RespaldosRiesgosDescartadosRFC.class,
                        match(where(OOAD_REGISTRO_PATRONAL).is(ooad).and(ANIO_REVISION).is(cicloActual).
                                and(CLAVE_ESTADO_RIESGO).is(
    ...
    public List<ParamModel> getCausisticaOtrosAniosPorTipoRiesgo(List<Integer> ciclosAnteriores, List<Integer> ciclosPosteriores, int ooad, List<Integer> subdelegaciones) {
        ...
        AggregationResults<BasicDBObject> aggregationResultsAnteriores = operations.aggregate(newAggregation(RespaldosRiesgosDescartadosRFC.class,
                        match(where(OOAD_REGISTRO_PATRONAL).is(ooad).and(ANIO_CICLO_CASO).in(ciclosAnteriores).and(SUB_DEL_REGISTRO_PATRONAL).in(subdelegaciones)
        ...
        BasicDBObject basicDBObject = null;
        if(aggregationResultsAnteriores.getMappedResults().isEmpty()){
            basicDBObject = new BasicDBObject();
            basicDBObject.append(TOTAL,"0");
            basicDBObject.append(DIAS_SUBSIDIADOS,"0");
            basicDBObject.append(PORCENTAJE_INCAPACIDAD,"0");
            ...
            basicDBObject = aggregationResultsAnteriores.getMappedResults().get(0);
        }
        ...
        AggregationResults<BasicDBObject> aggregationResultsPosteriores = operations.aggregate(newAggregation(RespaldosRiesgosDescartadosRFC.class,
                        match(where(OOAD_REGISTRO_PATRONAL).is(ooad).and(ANIO_CICLO_CASO).in(ciclosPosteriores).and(SUB_DEL_REGISTRO_PATRONAL).in(subdelegaciones)
    ...
    public Map<String, List<ParamModel>> getAccidentesEnfermedadesTrabajoAnioRevisionRegistroPatronal(int ooad, int cicloActual, List<Integer> subdelegaciones) {
        ...
        AggregationResults<BasicDBObject> aggregacion = operations.aggregate(newAggregation(RespaldoCierreAnualCasuisticaRFC.class,
                        match(where(OOAD_REGISTRO_PATRONAL).is(ooad).and(ANIO_REVISION).is(cicloActual).and(TIPO_RIESGO).in(
                                Arrays.asList(1, 3)
                        ).and(SUB_DEL_REGISTRO_PATRONAL).in(subdelegaciones)),
                        group(REGISTRO_PATRONAL)
                                .sum(DIAS_SUBSIDIADOS).as(DIAS_SUBSIDIADOS)
                                .sum(PORCENTAJE_INCAPACIDAD).as(PORCENTAJE_INCAPACIDAD)
                                .count().as(TOTAL)
                                .sum(ConditionalOperators.Cond.newBuilder()
        ...
        aggregacion.getMappedResults().stream().forEach(registro -> {
            List<ParamModel> params = new ArrayList<>();
